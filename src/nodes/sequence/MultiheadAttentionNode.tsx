...
export class MultiheadAttentionNode {
    ...
    static paramSchema: Record<string, FieldSpec> = {
        embed_dim: { required: true, type: "number", label: "Embed Dim", defaultValue: 128, step: 1 },
        num_heads: { required: true, type: "number", label: "Heads", defaultValue: 8, step: 1 }
    };
    static handles = { targets: ["query", "key", "value", "mask"], sources: ["out-0"] };

    static shapeVerifier(data: MhaData, inputShapes: number[][]) {
        if (inputShapes.length < 1 || inputShapes.length > 4) return { ok: false as const, error: "Expect 1-4 inputs (q[,k,v,mask])" };
        const q = inputShapes[0];
        if (q.length !== 3) return { ok: false as const, error: "Query must be [batch, seq, dim]" };
        const embed = getParamValue(this, data, "embed_dim") as number;
        const heads = getParamValue(this, data, "num_heads") as number;
        if (q[2] !== embed) return { ok: false as const, error: `Expected embed_dim ${embed}, got ${q[2]}` };
        if (heads <= 0 || embed % heads !== 0) return { ok: false as const, error: "embed_dim must be divisible by num_heads" };
        const kShape = inputShapes[1] || q;
        const vShape = inputShapes[2] || q;
        if (JSON.stringify(kShape) !== JSON.stringify(q)) return { ok: false as const, error: "Key must match query shape" };
        if (JSON.stringify(vShape) !== JSON.stringify(q)) return { ok: false as const, error: "Value must match query shape" };
        const mask = inputShapes[3];
        if (mask) {
            if (mask.length === 3) {
                const [b, t, s] = mask;
                if (b !== q[0] || t !== q[1] || s !== kShape[1]) return { ok: false as const, error: "Mask must be [batch, tgt_seq, src_seq]" };
            } else if (mask.length === 2) {
                const [t, s] = mask;
                if (t !== q[1] || s !== kShape[1]) return { ok: false as const, error: "Mask must match seq lengths" };
            } else {
    ...
    static Component = createLayerComponent<MhaData>(MultiheadAttentionNode.label, MultiheadAttentionNode.paramSchema, {
        handles: { targets: ["query", "key", "value", "mask"], sources: ["out-0"] }
    });
}
