...
export class AccuracyNode {
  ...
  static handleSchema: HandleSchema<MetricData> = {
    inputs: [
      { id: "logits", type: "input", defaultLabel: "logits", position: 0 },
      { id: "target", type: "input", defaultLabel: "target", position: 1 },
    ],
    outputs: [
      { id: "out", type: "output", defaultLabel: "accuracy", position: 0 },
    ],
  };
  ...
  static shapeVerifier(_data: MetricData, inputShapes: number[][]) {
    if (inputShapes.length !== 2)
      return {
        ok: false as const,
        error: "Accuracy expects logits and target",
      };
    const logits = inputShapes[0];
    const target = inputShapes[1];
    if (logits.length !== 2 && logits.length !== 3)
      return {
        ok: false as const,
        error: "Logits must be [batch, classes] or [batch, seq, classes]",
      };
    if (logits[0] !== target[0])
      return { ok: false as const, error: "Batch size mismatch" };
  ...
  static Component = createLayerComponent<MetricData>(
    AccuracyNode.label,
    AccuracyNode.paramSchema,
    {
      handleSchema: AccuracyNode.handleSchema,
    }
  );
}
