...
export class MatMulNode {
  ...
  static handleSchema: HandleSchema<MatMulData> = {
    inputs: [
      { id: "in-0", type: "input", defaultLabel: "a", position: 0 },
      { id: "in-1", type: "input", defaultLabel: "b", position: 1 },
    ],
    outputs: [
      { id: "out", type: "output", defaultLabel: "matmul", position: 0 },
    ],
  };

  static shapeVerifier(_data: MatMulData, inputShapes: number[][]) {
    if (inputShapes.length !== 2)
      return { ok: false as const, error: "MatMul expects two inputs" };
    const [a, b] = inputShapes;
    if (a.length < 2 || b.length < 2)
      return { ok: false as const, error: "Inputs must be at least 2D" };
    const aK = a[a.length - 1];
    const bK = b[b.length - 2];
    if (aK !== bK)
      return {
        ok: false as const,
        error: `Inner dims must align (${aK} vs ${bK})`,
      };
  ...
  static Component = createLayerComponent<MatMulData>(
    MatMulNode.label,
    MatMulNode.paramSchema,
    { handleSchema: MatMulNode.handleSchema }
  );
}
