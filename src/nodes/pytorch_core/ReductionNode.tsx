  ...
  return class ReductionNode {
    ...
    static paramSchema: Record<string, FieldSpec> = {
      dim: {
        required: true,
        type: "number",
        label: "Dim (default last)",
        defaultValue: -1,
        step: 1,
      },
    };

    static handleSchema: HandleSchema<ReduceData> = {
      inputs: [{ id: "in", type: "input", defaultLabel: "x", position: 0 }],
      outputs: [
        { id: "out", type: "output", defaultLabel: "reduced", position: 0 },
      ],
    };
    ...
    static shapeVerifier(data: ReduceData, inputShapes: number[][]) {
      ...
      const axis = dim < 0 ? rank + dim : dim;
      if (axis < 0 || axis >= rank)
        return { ok: false as const, error: `Dim must be in [0, ${rank - 1}]` };
    ...
    static Component = createLayerComponent<ReduceData>(
      label,
      ReductionNode.paramSchema,
      {
        handleSchema: ReductionNode.handleSchema,
      }
    );
  };
}
