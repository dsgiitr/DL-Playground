...
export class CrossEntropyLossNode {
  ...
  static shapeVerifier(_data: LossData, inputShapes: number[][]) {
    if (inputShapes.length !== 2)
      return {
        ok: false as const,
        error: "CrossEntropy expects logits and target",
      };
    ...
    if (target[0] !== batch)
      return {
        ok: false as const,
        error: "Target batch must match logits batch",
      };
  ...
  static Component = createLayerComponent<LossData>(
    CrossEntropyLossNode.label,
    CrossEntropyLossNode.paramSchema,
    {
      handleSchema: CrossEntropyLossNode.handleSchema,
    }
  );
}
