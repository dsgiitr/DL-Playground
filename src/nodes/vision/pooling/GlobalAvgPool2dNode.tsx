...
// Convenience wrapper for adaptive pool to a single spatial bin.
export class GlobalAvgPool2dNode {
    static label = "GlobalAvgPool2d";
    static paramSchema: Record<string, FieldSpec> = {};

    static shapeVerifier(_data: GlobalPoolData, inputShapes: number[][]) {
        if (inputShapes.length !== 1) return { ok: false as const, error: "GlobalAvgPool2d expects one input" };
    ...
    static Component = createLayerComponent<GlobalPoolData>(GlobalAvgPool2dNode.label, GlobalAvgPool2dNode.paramSchema);
}
