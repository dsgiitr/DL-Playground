...
export class MaxPool1dNode {
    static label = "MaxPool1d";
    ...
    static shapeVerifier(data: PoolData, inputShapes: number[][]) {
        if (inputShapes.length !== 1) return { ok: false as const, error: "MaxPool1d expects exactly one input" };
    ...
    static estimateCost(data: PoolData, _inputShapes: number[][], outputShape: number[]) {
        ...
        return estimatePoolCost(outputShape, k);
    }

    static getInitCode(data: PoolData, name: string) {
        const k = getParamValue(MaxPool1dNode.paramSchema, data, "kernel_size");
        const s = getParamValue(MaxPool1dNode.paramSchema, data, "stride");
        return `self.${name} = nn.MaxPool1d(kernel_size=${k}, stride=${s})`;
    }
    ...
    static Component = createLayerComponent<PoolData>(MaxPool1dNode.label, MaxPool1dNode.paramSchema);
}
