...
export class AdaptiveAvgPool2dNode {
    static label = "AdaptiveAvgPool2d";
    ...
    static shapeVerifier(data: AdaptivePoolData, inputShapes: number[][]) {
        if (inputShapes.length !== 1) return { ok: false as const, error: "AdaptiveAvgPool2d expects one input" };
    ...
    static getInitCode(data: AdaptivePoolData, name: string) {
        const out = getParamValue(AdaptiveAvgPool2dNode.paramSchema, data, "output_size");
        return `self.${name} = nn.AdaptiveAvgPool2d(${out})`;
    }
    ...
    static Component = createLayerComponent<AdaptivePoolData>(AdaptiveAvgPool2dNode.label, AdaptiveAvgPool2dNode.paramSchema);
}
