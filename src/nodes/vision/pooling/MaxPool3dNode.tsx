...
export class MaxPool3dNode {
    static label = "MaxPool3d";
    ...
    static shapeVerifier(data: PoolData, inputShapes: number[][]) {
        if (inputShapes.length !== 1) return { ok: false as const, error: "MaxPool3d expects exactly one input" };
    ...
    static getInitCode(data: PoolData, name: string) {
        const k = getParamValue(MaxPool3dNode.paramSchema, data, "kernel_size");
        const s = getParamValue(MaxPool3dNode.paramSchema, data, "stride");
        return `self.${name} = nn.MaxPool3d(kernel_size=${k}, stride=${s})`;
    }
    ...
    static Component = createLayerComponent<PoolData>(MaxPool3dNode.label, MaxPool3dNode.paramSchema);
}
