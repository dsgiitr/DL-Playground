...
export class AvgPool2dNode {
    ...
    static shapeVerifier(data: PoolData, inputShapes: number[][]) {
        if (inputShapes.length !== 1) return { ok: false as const, error: "AvgPool2d expects exactly one input" };
        const shape = inputShapes[0];
        if (shape.length !== 4) return { ok: false as const, error: "AvgPool2d input must be [batch, channels, height, width]" };
        const [, , h, w] = shape;
