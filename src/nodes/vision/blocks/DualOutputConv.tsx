...
/**
 * Demo multi-output node: Applies Conv2d and outputs both the conv result
...
export class DualOutputConvNode {
    ...
    static paramSchema: Record<string, FieldSpec> = {
        in_channels: { type: "number", required: true, defaultValue: 3, label: "In Channels" },
        out_channels: { type: "number", required: true, defaultValue: 64, label: "Out Channels" },
        kernel_size: { type: "number", required: true, defaultValue: 3, label: "Kernel Size" },
    };
    ...
    static shapeVerifier(data: DualOutputConvData, inputShapes: number[][]) {
        ...
        if (input.length !== 4) {
            return { ok: false as const, error: "DualConv expects 4D input [B, C, H, W]" };
        }
        return { ok: true as const };
    }

    // Return per-handle shapes
    static shapeCompute(data: DualOutputConvData, inputShapes: number[][]): Record<string, number[]> {
        const [B, _C, H, W] = inputShapes[0];
        const out_channels = data.out_channels ?? this.paramSchema.out_channels.defaultValue;
        const kernel_size = data.kernel_size ?? this.paramSchema.kernel_size.defaultValue;
    ...
    static getInitCode(data: DualOutputConvData, name: string): string {
        const in_channels = data.in_channels ?? this.paramSchema.in_channels.defaultValue;
        const out_channels = data.out_channels ?? this.paramSchema.out_channels.defaultValue;
        const kernel_size = data.kernel_size ?? this.paramSchema.kernel_size.defaultValue;
