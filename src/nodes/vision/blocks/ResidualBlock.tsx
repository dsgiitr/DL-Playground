...
export class ResidualBlockNode {
  ...
  static handleSchema: HandleSchema<ResidualData> = {
    inputs: [{ id: "in", type: "input", defaultLabel: "x", position: 0 }],
    outputs: [
      { id: "out_main", type: "output", defaultLabel: "out_main", position: 0 },
      { id: "out_skip", type: "output", defaultLabel: "out_skip", position: 1 },
    ],
  };
  ...
  static shapeVerifier(data: ResidualData, inputShapes: number[][]) {
    if (inputShapes.length !== 1)
      return {
        ok: false as const,
        error: "Residual block expects exactly one input",
      };
    const shape = inputShapes[0];
    if (shape.length < 3)
      return { ok: false as const, error: "Expected NCHW or similar tensor" };
    const channels = getParamValue(this, data, "channels") as number;
    if (shape[1] !== channels)
      return {
        ok: false as const,
        error: `Expected ${channels} channels, got ${shape[1]}`,
      };
  ...
  static Component = createLayerComponent<ResidualData>(
    ResidualBlockNode.label,
    ResidualBlockNode.paramSchema,
    {
      handleSchema: ResidualBlockNode.handleSchema,
    }
  );
}
