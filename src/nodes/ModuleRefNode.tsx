...
function toHandleSchema(
  contract?: ModuleContract
): HandleSchema<ModuleRefData> {
  if (!contract) {
    return {
  ...
  const outputs = contract.outputs?.length ? contract.outputs : ["out"];
  return {
    inputs: inputs.map((id, i) => ({
      id,
      type: "input" as const,
      defaultLabel: id,
      position: i,
    })),
    outputs: outputs.map((id, i) => ({
      id,
...
const ModuleRefComponent: ComponentType<NodeProps<ModuleRefData>> = ({
  id,
  data,
  isConnectable,
}) => {
        ...
        <div style={{ position: "absolute", inset: 0 }}>
          {isConnectable && (
            <div style={{ pointerEvents: "all" }}>
              {renderHandles("left", handles.targets)}
            </div>
          )}
          {isConnectable && (
            <div style={{ pointerEvents: "all" }}>
              {renderHandles("right", handles.sources)}
            </div>
          )}
        </div>
      </div>
...
export const ModuleRefNode: LayerDefinition<ModuleRefData> = {
  ...
  diagramFamily: "block",
  paramSchema: {},
  handleSchema: (data: ModuleRefData) => toHandleSchema(data.contract),
  shapeVerifier: (data: ModuleRefData, inputShapes: number[][]) => {
    void data;
    void inputShapes;
    return { ok: true as const };
  },
  shapeCompute: (_data: ModuleRefData, inputShapes: number[][]) =>
    inputShapes?.[0] || [],
  getInitCode: (data: ModuleRefData, name: string) =>
    `# module ${data.name || data.moduleId || name}`,
  getForwardCode: (
    _data: ModuleRefData,
    _name: string,
    inputs: Array<string>,
