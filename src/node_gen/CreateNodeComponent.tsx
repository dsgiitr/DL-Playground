...
export function createLayerComponent<D extends LayerData>(
  ...
  return ({ id, data, isConnectable }: NodeProps<Node<any>>) => {
    ...
    const hiddenOptionCount =
      optionalParams.length - (renderList.length - requiredParams.length);

    const shapePreview = (() => {
      const liveShape = (safeData as any).__shape as
        | number[]
        | Record<string, number[]>
        | undefined;
      if (Array.isArray(liveShape) && liveShape.length > 0)
    ...
    const resolvedHandles: HandleSpec = (() => {
      ...
      if (schema) {
        const resolved =
          typeof schema === "function" ? schema(safeData) : schema;
    ...
    return (
      ...
      >
        ...
        <div
          style={{ padding: "0 10px 10px", fontSize: "10px", color: "#888" }}
        >
          Shape: {shapePreview}
        </div>
