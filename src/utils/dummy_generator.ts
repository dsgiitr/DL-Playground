...
export function generatePyTorchCode(nodes: Node[], edges: Edge[]) {
    ...
    sortedNodes.forEach((node, index) => {
        ...
        if (type && LAYER_REGISTRY[type]) {
            ...
            const inputNames =
                inEdges.length === 0
                    ? ["x"]
                    : inEdges.map(e => (e.data as any)?.label as string);
            ...
            const outputNames =
                outEdges.length === 0
                    ...
                    : outEdges.map(e => (e.data as any)?.label as string);
    ...
    if (terminalNodes.length === 1) {
        const lastOut = outgoingEdges[terminalNodes[0].id][0]?.data?.label;
        forwardBlock += `        return ${lastOut ?? "out_" + terminalNodes[0].id}\n`;
    } else {
        const returns = terminalNodes.map(n =>
            outgoingEdges[n.id][0]?.data?.label ?? `out_${n.id}`
        );
