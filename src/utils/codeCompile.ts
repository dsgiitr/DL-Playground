    ...
    nodes.forEach(child => {
        ...
        if (child.type && child.type === "module_ref") {
            ...
            if (color[module_id] === 1) {
                ...
                const internalNodes : Node[] = module_data.contract?.internalNodes || [];
...
export function recursiveCodeGenerator (nodes: Node[], edges: Edge[]) : CodeGenResult {
    ...
    order.forEach(moduleId => {
        ...
        if (moduleId === "0") { // Accidental clash?
            ...
            if (savedModule && savedModule.contract) {
                moduleNodes = savedModule.contract.internalNodes;
                moduleEdges = savedModule.contract.internalEdges;
