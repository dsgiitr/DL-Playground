...
function createLayerComponent<D extends LayerData>(label: string, fields: string[], shapeFn: (data: D) => number[]) {
    return ({ id, data, isConnectable }: NodeProps<Node<any>>) => {
        ...
        return (
            ...
            >
                ...
                <div style={{ marginTop: "5px", fontSize: "10px" }}>Shape: {JSON.stringify(shapeFn(data))}</div>
...
export class LinearLayerNode {
    ...
    static fields = ["inFeatures", "outFeatures"];

    static computeShape(data: LinearData) {
        return [data.outFeatures];
    }
    ...
    static Component = createLayerComponent(
        LinearLayerNode.label,
        LinearLayerNode.fields,
...
//         this.data = data || ({} as D);
//     }
//     // These are instance level functions that need to be overriden
//     abstract get fields(): string[];
//     abstract computeShape(): number | number[];
//     abstract verifyInputShapes(data?: D): boolean;
