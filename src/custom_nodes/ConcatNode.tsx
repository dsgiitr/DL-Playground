...
export class ConcatNode {
    ...
    static paramSchema: Record<string, FieldSpec> = {
        dim: { required: true, type: "number", label: "Axis", defaultValue: 1, step: 1 },
    };

    static shapeVerifier(data: ConcatData, inputShapes: number[][]) {
        if (inputShapes.length < 2) return { ok: false as const, error: "Concat needs at least two inputs" };
        const rank = inputShapes[0].length;
        const dim = (data.dim ?? this.paramSchema.dim.defaultValue) as number;
        if (dim < 0 || dim >= rank) return { ok: false as const, error: `Axis must be between 0 and ${rank - 1}` };

        for (let i = 1; i < inputShapes.length; i++) {
            const shp = inputShapes[i];
            if (shp.length !== rank) return { ok: false as const, error: "All inputs must have the same rank" };
            for (let d = 0; d < rank; d++) {
                if (d === dim) continue;
                if (shp[d] !== inputShapes[0][d]) {
                    return { ok: false as const, error: "Concat requires matching dims on all axes except concat axis" };
                }
            }
        }
    ...
    static getForwardCode(_data: ConcatData, _name: string, inputs: Array<string>, outputs: Array<string>) {
